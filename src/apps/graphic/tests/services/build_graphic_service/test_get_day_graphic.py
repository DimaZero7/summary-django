...
class GetDayGraphicTest(TestCase):
    def test_values_less_than_points(self):
        # Create data
        count = BuildGraphicService.POINTS_FOR_DAY - 1
        ...
        for iteration, change_share_price in enumerate(change_shares_price):
            change_share_price.created_timestamp -= timedelta(hours=iteration)
            change_share_price.save(update_fields=["created_timestamp"])

        # Action
        result = BuildGraphicService().get_day_graphic()
    ...
    def test_values_more_than_points(self):
        ...
        # Check
        self.assertEqual(len(result), BuildGraphicService.POINTS_FOR_DAY)

    @freeze_time(datetime(2024, 1, 1, 23, 0))
    def test_result_values(self):
        ...
        one_points_data = [
            (one_point_price, datetime(2024, 1, 1, 1, 0, tzinfo=timezone.utc)),
            (
                one_point_price,
        ...
        two_points_data = [
            (two_point_price, datetime(2024, 1, 1, 2, 0, tzinfo=timezone.utc)),
            (
                two_point_price,
                datetime(2024, 1, 1, 2, 10, tzinfo=timezone.utc),
            ),
            (
                two_point_price,
                datetime(2024, 1, 1, 2, 20, tzinfo=timezone.utc),
            ),
        ]
